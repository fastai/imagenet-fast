...
import fastai, fastai.dataloader, fastai.core
...
def main():
    ...
    #tfms = tfms_from_stats(imagenet_stats, 224, transforms_side_on, max_zoom=1.1)
    tfms = tfms_from_stats(imagenet_stats, 224, max_zoom=1.1)
    ttfms,vtfms = tfms
    ...
    #vtfms.tfms = [to_array] + vtfms.tfms# + [TT]

    data = ImageClassifierData.from_paths(args.data, val_name='val', tfms=tfms, bs=args.batch_size,
                                          num_workers=args.workers)

    normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
    ttfms = transforms.Compose([
        transforms.RandomResizedCrop(args.sz),
        transforms.RandomHorizontalFlip(),
        transforms.ToTensor(),
        normalize,
    ])
    vtfms = transforms.Compose([
        transforms.Resize(int(args.sz*1.14)),
        transforms.CenterCrop(args.sz),
        transforms.ToTensor(),
        normalize,
    ])
    train_dataset = datasets.ImageFolder( traindir,ttfms )

    train_sampler = (torch.utils.data.distributed.DistributedSampler(train_dataset)
                     if args.distributed else None)

    train_loader = torch.utils.data.DataLoader(
    #train_loader = fastai.dataloader.DataLoader(
        train_dataset, batch_size=args.batch_size, shuffle=(train_sampler is None),
        #data.trn_ds, batch_size=args.batch_size, shuffle=(train_sampler is None),
        num_workers=args.workers, pin_memory=True, sampler=train_sampler)

    val_loader = torch.utils.data.DataLoader(
    #val_loader = fastai.dataloader.DataLoader(
        datasets.ImageFolder(valdir,vtfms),
        #data.val_ds,
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    prefetcher = data_prefetcher(train_loader, prefetch=False)
