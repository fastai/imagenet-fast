...
class DarknetBlock(nn.Module):
    def __init__(self, ch_in, hid_f=0.5):
        super().__init__()
        ch_hid = int(ch_in*hid_f)
...
class Darknet(nn.Module):
    ...
    def make_group_layer(self, ch_in, num_blocks, stride=1):
        ...
        for i in range(num_blocks): layers.append(DarknetBlock(ch_in*2, self.hid_f))
        return layers

    def __init__(self, num_blocks, num_classes=1000, start_nf=32, hid_f=0.5):
        super().__init__()
        nf = start_nf
        self.hid_f = hid_f
...
def darknet(num_classes=10): return Darknet([4,12,12], num_classes, hid_f=0.5)
